...
type AcmDb = KeyMaterialDb;
type S3Db = KeyMaterialDb;
...
impl DbSource {
    pub fn new(source_config: config::Source) -> Result<Self, Error> {
        ...
        match source_config {
            ...
            config::Source::S3 {
                ...
                Ok(Self::S3 {
                    db: Self::fetch_s3_db(bucket_arn.as_str(), path.as_str())?,
    ...
    pub fn update(&mut self) -> Result<bool, Error> {
        match self {
            ...
            Self::S3 {
                ...
                let new_db = Self::fetch_s3_db(bucket_arn, path)?;
                let res = new_db != *db;
                *db = new_db;
                Ok(res)
            },
    ...
    fn fetch_s3_db(bucket_arn: &str, path: &str) -> Result<AcmDb, Error> {
        debug!("Fetching cert from s3: {}/{}", bucket_arn, path);
        let s3_url = format!(
            "s3://{}/{}",
            bucket_arn,
            path,
            ...
            Some(config::Target::Conf {
                ...
                let post_action = match self.service {
                    ManagedService::Nginx => {
                        ...
                        if restart_hint {
                            Some(PostSyncAction::RestartNginx)
                        } else {
                            None
                        }
                    }
